app.factory('imports', ['$q', function($q) {
    ...
    (function( $ ) {
      ...
      $.csv = {
        ...
        toObjects: function(csv, options, callback) {
          ...
          var config = {};
          config.callback = ((callback !== undefined && typeof(callback) === 'function') ? callback : false);
          config.headerCallback = (( options.headerCallback !== undefined && typeof( options.headerCallback) === 'function') ?  options.headerCallback : false);
          config.separator = 'separator' in options ? options.separator : $.csv.defaults.separator;
          config.delimiter = 'delimiter' in options ? options.delimiter : $.csv.defaults.delimiter;
          ...
          if(config.headerCallback){
            headers = config.headerCallback(headers);
          }
    ...
    var imports = {
        ...
        csv: function(data) {
            ...
            //Read the text file
            var file = data.items[0].webkitGetAsEntry();

            file.file(function(file) {
                var reader = new FileReader();

                reader.onload = function(e) {
                    var objects = jQuery.csv.toObjects(e.target.result, {
                        headerCallback : function(columns){
                            if(columns.length){
                ...
                reader.onerror = function(e) {
                    deferred.reject('File was not loaded.');
                };
